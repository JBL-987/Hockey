...
(function () {
    ...
    Hockey.GameScene.prototype = {
        init: function (model) {
            ...
            this.camera = new THREE.PerspectiveCamera(40, window.innerWidth / window.innerHeight, 0.5, 1000);
        ...
        loadBackground: function () {
            var t = THREE.ImageUtils.loadTexture("images/Platform.jpg");
            t.wrapS = THREE.RepeatWrapping;
            t.wrapT = THREE.RepeatWrapping;
            var material = new THREE.ShaderMaterial({
                uniforms: { texture: { type: "t", value: t }, innerRadius: { type: "f", value: 0.3 }, outerRadius: { type: "f", value: 0.9 } },
                vertexShader: document.getElementById("vs_floor").textContent,
                fragmentShader: document.getElementById("fs_floor").textContent,
            });
            var floorPlane = new THREE.PlaneGeometry(8, 8);
        ...
        loadSkybox: function () {
            ...
            var materialArray = [];
            for (var i = 0; i < 6; i++) materialArray.push(new THREE.MeshBasicMaterial({ map: THREE.ImageUtils.loadTexture(imagePrefix + directions[i] + imageSuffix), side: THREE.BackSide }));
        ...
        loadModels: function (callback) {
            ...
            loader.load("models/table.obj", "models/table.mtl", function (object) {
                ...
                var tableSize = { width: bb.max.z - bb.min.z, depth: bb.max.x - bb.min.x, height: bb.max.y - bb.min.y };
                var scale = 1.75 / tableSize.width;
                tableSize.width *= scale * 0.87;
                tableSize.depth *= scale * 0.93;
                tableSize.height *= scale * 0.93;
        ...
        onGoal: function (puck) {
            ...
            puck.mesh.traverse(function (object) {
                object.visible = !1;
            });
            puck.active = !1;
            this.updateScores();
        ...
        updateScores: function () {
            ...
            document.getElementById("playerB_score").innerHTML = "Com: " + this.model.playerB.score;
        },
        update: function () {
            var model = this.model;
            ...
            if (paddle.position.z + paddleRadius > tableSize.depth * (0.5 - model.goalSize.depth) && Math.abs(paddle.position.x) < tableSize.width * model.goalSize.width * 0.5 + paddleRadius) {
